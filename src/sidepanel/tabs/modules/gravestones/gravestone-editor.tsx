...
export function GravestoneEditor() {
    ...
    return (
        <>
            ...
            {open && <GravestoneEditorWindow onClose={() => setOpen(false)} />}
        </>
    );
}

function GravestoneEditorWindow({ onClose }: { onClose: () => void }) {
    ...
    const onGridClick = (row: number, col: number) => {
        console.log(row, col, selectedTool, selectedGrave);
        if (selectedTool == 'none') return;

        if (selectedTool == 'place') {
            levelBuilder.tileManager.setTile({ row, col }, { type: 'gravestone', variant: selectedGrave });
        }

        if (selectedTool == 'remove') {
            levelBuilder.tileManager.removeTile({ row, col }, { type: 'gravestone' });
        }
    };

    useEffect(() => {
        setGridListener(onGridClick);
        return () => {
            setGridListener(undefined);
        };
    }, [selectedGrave, selectedTool]);

    const gravestones: Record<string, { label: string; icon?: string }> = {
                ...
                <Select value={selectedGrave} onValueChange={setSelectedGrave}>
                    ...
                    <SelectContent className="z-99999" position="popper">
                        <SelectGroup>
                            ...
                            {Object.entries(gravestones).map(([key, data]) => (
